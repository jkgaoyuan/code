##### 将adhaoc的内容 写成 函数
#####3若ssh链接拒绝请查看 被控制的主机上是否存在 本机登录用户
import shutil
...
def adhoc(server_list, hosts, moudle, args):
    ...
    play_source =  dict(
        name="Ansible Play",
        hosts=hosts,  # 在哪些目标执行指令
        gather_facts='no',   # 执行指令前不收集目标主机信息
        tasks=[  # 调用哪个模块，添加哪些参数
            dict(action=dict(module=moudle, args=args), register='shell_out'),
...
if __name__ == '__main__':
    ...
    moudle = 'shell'
    args = 'id root'
    adhoc(server_list, hosts, moudle, args)